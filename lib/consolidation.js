...
function readLeastFundedAddresses(asset, wallet, handleFundedAddresses) {
	db.query("SELECT address, SUM(amount) AS total \n\
		...
		GROUP BY address ORDER BY SUM(amount) LIMIT 15", [wallet], function (rows) {
		handleFundedAddresses(rows.map(function (row) {
			return row.address;
		}));
	});
}
...
function determineCountOfOutputs(asset, wallet, handleCount) {
	db.query("SELECT COUNT(*) AS count FROM my_addresses CROSS JOIN outputs USING(address) JOIN units USING(unit) \n\
		WHERE wallet=? AND is_spent=0 AND " + (asset ? "asset=" + db.escape(asset) : "asset IS NULL") + " AND is_stable=1 AND sequence='good'", [wallet], function (rows) {
		handleCount(rows[0].count);
	});
}

function readDestinationAddress(wallet, handleAddress) {
	db.query("SELECT address FROM my_addresses WHERE wallet=? ORDER BY is_change DESC, address_index ASC LIMIT 1", [wallet], function (rows) {
...
function consolidate(wallet, signer) {
	var asset = null;
	mutex.lock(['consolidate'], function (unlock) {
		determineCountOfOutputs(asset, wallet, function (count) {
			console.log(count + ' unspent outputs');
			if (count <= conf.MAX_UNSPENT_OUTPUTS) return unlock();
			var count_to_spend = Math.min(count - conf.MAX_UNSPENT_OUTPUTS + 1, constants.MAX_INPUTS_PER_PAYMENT_MESSAGE - 1);
			readLeastFundedAddresses(asset, wallet, function (arrAddresses) {
				db.query("SELECT address, unit, message_index, output_index, amount \n\
					...
					ORDER BY amount LIMIT ?", [arrAddresses, count_to_spend], function (rows) {
					...
					function addLargeInputIfNecessary(onDone) {
						...
						db.query("SELECT address, unit, message_index, output_index, amount \n\
								...
								LIMIT 1", [wallet, target_amount - input_amount, Object.keys(assocUsedUnits)], function (large_rows) {
							if (large_rows.length === 0) return onDone("no large input found");
							var row = large_rows[0];
					...
					var assocUsedUnits = {};
					var input_amount = 0;
					var arrInputs = rows.map(function (row) {
						assocUsedAddresses[row.address] = true;
					...
					addLargeInputIfNecessary(function (err) {
						if (err) {
							console.log("consolidation failed: " + err);
							return unlock();
						}
						var arrUsedAddresses = Object.keys(assocUsedAddresses);
						readDestinationAddress(wallet, function (dest_address) {
							...
							composer.composeJoint({
								...
								callbacks: composer.getSavingCallbacks({
									...
									ifNotEnoughFunds: function ifNotEnoughFunds(err) {
